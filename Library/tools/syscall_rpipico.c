...
static void write_call(int n) {
  FILE *fp;
  int saves = 0;
  snprintf(namebuf, 128, "fuzixrpipico/syscall_%s.S", syscall_name[n]);
  fp = fopen(namebuf, "w");
  if (fp == NULL) {
    perror(namebuf);
    exit(1);
...
static void write_makefile(void) {
  int i;
  FILE *fp = fopen("fuzixrpipico/Makefile", "w");
  if (fp == NULL) {
    perror("writing Makefile");
    exit(1);
  }
  fprintf(fp, "# Autogenerated by tools/syscall_rpipico\n");
  fprintf(fp, "CROSS_AS=arm-none-eabi-as\nCROSS_AR=arm-none-eabi-ar\n");
  fprintf(fp, "ASOPTS=\n\n");
  fprintf(fp, "ASRCS = syscall_ret.S\n");
  for (i = 0; i < NR_SYSCALL; i++)
