 ...
 *
 *	8070 - Nice and simple as we use the compiler calling convention
...
{
  FILE *fp;
  snprintf(namebuf, 128, "fuzix8070/syscall_%s.s",syscall_name[n]);
  fp = fopen(namebuf, "w");
  if (fp == NULL) {
    perror(namebuf);
    exit(1);
...
static void write_makefile(void)
{
  ...
  fp = fopen("fuzix8070/Makefile", "w");
  if (fp == NULL) {
    perror("Makefile");
    exit(1);
  }
  fprintf(fp, "# Autogenerated by tools/syscall8070\n");
