...
static void write_call(int n) {
  FILE *fp;
  int saves = 0;
  snprintf(namebuf, 128, "fuzixesp32/syscall_%s.S", syscall_name[n]);
  fp = fopen(namebuf, "w");
  if (fp == NULL) {
    perror(namebuf);
    exit(1);
...
static void write_makefile(void) {
  int i;
  FILE *fp = fopen("fuzixesp32/Makefile", "w");
  if (fp == NULL) {
    perror("writing Makefile");
    exit(1);
  }
  fprintf(fp, "# Autogenerated by tools/syscall_esp32\n");
  fprintf(fp, "CROSS_AS=xtensa-esp32-elf-as\nCROSS_AR=xtensa-esp32-elf-ar\n");
  fprintf(fp, "ASOPTS=\n\n");
