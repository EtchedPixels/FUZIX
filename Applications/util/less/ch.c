...
fch_get()
{
    ...
    while ((n = read(file, &bp->data[end], PBUFSIZ-end)) > 0) {
	if ((end += n) >= PBUFSIZ) {
	    break;
	}
    }
    ...
    /*
     * Set an CEOF marker in the buffered data itself.
     * Then ensure the data is "clean": there are no 
     * extra CEOF chars in the data and that the "meta"
    ...
    register POSITION pos;
{
    ...
    new_block = pos / PBUFSIZ;
    if (!ispipe || (new_block == last_piped_block + 1) ||
	    buffered(new_block)) {
	/*
	 * Set read pointer.
	 */
	ch_block = new_block;
	ch_offset = pos % PBUFSIZ;
	return (0);
    }
    return (1);
}
...
ch_tell()
{
    return (ch_block * PBUFSIZ + ch_offset);
}
