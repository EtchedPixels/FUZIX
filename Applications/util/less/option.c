 ...
 */
int p_nbufs, f_nbufs;	/* Number of buffers.  There are two values,
				   one used for input from a pipe and 
				   the other for input from a file. */
int clean_data;		/* Can we assume the data is "clean"? 
				   (That is, free of nulls, etc) */
int quiet;		/* Should we suppress the audible bell? */
int top_search;		/* Should forward searches start at the top 
...
int twiddle;		/* Display "~" for lines after EOF */
...
char all_options[64];	/* List of all valid options */
