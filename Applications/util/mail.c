...
/* From MINIX : See MINIX license in CREDITS */
...
char mailbox[MAIL_PATHLEN];		/* user's mailbox/maildrop */
...
int tempfd;			/* TODO: optimize the various re-opens to use this */
...
int main(int argc, char *argv[])
{
    ...
    /* Protect against being run with stdout/stderr closed or similar
       whilst setuid */
    if (dup(0) != 3)
        _exit(1);
    else
    ...
    /* name the temp file and open it. We end up with an open handle
       to the file we keep */
    if ((tempfd = mkstemp(tempname)) == -1) {
        fprintf(stderr, "mail: unable to create temporary file.\n");
        exit(1);
    }
    ...
    while (EOF != (c = getopt(argc, argv, "epqrf:tdvs:"))) switch (c) {
        ...
        case 'f':
                        if (setuid(getuid()) == -1)	/* won't need to lock */
                            _exit(1);
                        usedrop = 0;
                        strncpy(mailbox, optarg, (size_t)(MAIL_PATHLEN - 1));
                        break;
    ...
    if (usedrop) snprintf(mailbox, MAIL_PATHLEN, DROPNAME, whoami());
...
int deliver(int count, char *vec[])
{
    ...
    char lockname[MAIL_PATHLEN];	/* maildrop lock */
    ...
    for (i = 0; i < count; ++i) {
        ...
        snprintf(mailbox, MAIL_PATHLEN, DROPNAME, pw->pw_name);
        snprintf(lockname, MAIL_PATHLEN, LOCKNAME, pw->pw_name);
...
void updatebox(void)
{
    ...
    snprintf(lockname, MAIL_PATHLEN, LOCKNAME, whoami());
