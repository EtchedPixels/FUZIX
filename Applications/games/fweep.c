...
void zch_print(int z) {
  ...
  if(zch_shift==3) {
    ...
    zch_shift=5;
  } else if((z==4 || z==5) && VERSION>2 && (zch_shift==1 || zch_shift==2)) {
    ...
    if(alphabet_table) char_print(read8(alphabet_table+z+(zch_shift*26)-6));
...
void make_rectangle(uint32_t addr,int width,int height,int skip) {
  ...
  int w,h;
  for(h=0;h<height;h++) {
    for(w=0;w<width;w++) char_print(read8(addr++));
    addr+=skip;
    if(h!=height-1) {
      char_print(13);
...
void exit_routine(uint16_t result) {
  ...
  if(frames[frameptr+1].stored) store(read8(program_counter-1),result);
}

void branch(uint32_t cond) {
  int v=pc();
  if(!(v&0x80)) cond=!cond;
  if(v&0x40) v&=0x3F; else v=((v&0x3F)<<8)|pc();
  if(cond) {
...
uint64_t dictionary_encode(uint8_t*text,int len) {
  ...
  const uint8_t*al=(alphabet_table?(const uint8_t*)memory+alphabet_table:(const uint8_t*)(VERSION>1?v2alpha:v1alpha));
  while(c && len && *text) {
...
void add_to_parsebuf(uint32_t parsebuf,uint32_t dict,uint8_t*d,int k,int el,int ne,int p,uint16_t flag) {
  ...
  for(i=0;i<ne;i++) {
    ...
    if(g==v) {
      /* FIXME tidy re-uses */
      write8(parsebuf+(read8(parsebuf+1)<<2)+5, p+1+(VERSION>4));
      write8(parsebuf+(read8(parsebuf+1)<<2)+4, k);
      write16(parsebuf+(read8(parsebuf+1)<<2)+2,dict);
      break;
    }
    dict+=el;
  }
...
void tokenise(uint32_t text,uint32_t dict,uint32_t parsebuf,int len,uint16_t flag) {
  boolean ws[256];
  uint8_t d[10];
  int i,el,ne,k,p,p1;
  ...
  dict+=memory[dict]+4;
  while(p<len && read8(text+p) && read8(parsebuf+1) < read8(parsebuf)) {
    i=memory[text+p];
    if(i>='A' && i<='Z') i+='a'-'A';
    if(i=='?' && qtospace) i=' ';
    if(i==' ') {
...
void game_restore(void) {
  ...
  while(o<static_start) {
    ...
    if(d) {
      ...
      while(c-->=0) write8(o++, fgetc(story));
    }
  }
  fclose(fp);
  while(o<static_start) write8(o++, fgetc(story));
  program_counter=frames[frameptr].pc;
}
...
void execute_instruction(void) {
  ...
  if(in&0x80) {
    if(in>=0xC0 || in==0xBE) {
      ...
      if(in==0xBE) in=pc();
      at=pc()<<8;
      if(in==0xEC || in==0xFA) at|=pc(); else at|=0x00FF;
  ...
  switch(in) {
    ...
    case 0x04: // Set font
      ...
      if(!tandy) putchar(*inst_args==3?14:15);
      break;
    ...
    case 0x84: // Property length
      ...
      storei(VERSION<4?(in>>5)+1:in&0x80?(in&63?(in & 63):64):(in>>6)+1);
      break;
    case 0x85: // Increment
    ...
    case 0xCA: // Test attributes
      branch(read8(attribute(*inst_args)+(inst_args[1]>>3))&(0x80>>(inst_args[1]&7)));
      break;
    ...
    case 0xD1: // Read property
      if(u=property_address(inst_args[0],inst_args[1])) storei(cur_prop_size==1?read8(u):read16(u));
    ...
    case 0xF7: // Scan a table
      ...
      u=inst_args[1];
      while(inst_args[2]) {
        if(*inst_args==(inst_args[3]&0x80?read16(u):read8(u))) break;
        u+=inst_args[3]&0x7F;
        inst_args[2]--;
      }
    ...
    case 0xFD: // Copy a table
      if(!inst_args[1]) {
        ...
        // backward!
        m=inst_sargs[2];
        while(m--) write8(inst_args[1]+m, read8(inst_args[0]+m));
      } else {
        // forward!
        if(inst_sargs[2]<0) inst_sargs[2]*=-1;
        m=0;
        while(m<inst_sargs[2])
          write8(inst_args[1]+m, read8(inst_args[0]+m),m);
          m++;
      }
      break;
...
void game_begin(void) {
  ...
  switch(VERSION) {
    ...
    case 2:
      ...
      break;
    case 3:
      write8low(0x01, (read8low(0x01)&0x8F)| 0x10 | (tandy << 3));
      break;
    case 4:
      write8low(0x01, 0x00);
      break;
  ...
  if(VERSION>1) synonym_table=read16low(0x18);
  if(VERSION>3) {
  ...
  if(!(read8low(2)&128)) write16low(0x02,0x0802);
