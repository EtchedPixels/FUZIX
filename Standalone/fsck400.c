...
#define SMOUNTED_WRONGENDIAN 50993U   /* byteflipped */
...
static void pass1(void)
{
    ...
    for (n = ROOTINODE; n < 6 * (swizzle16(superblock.s_isize) - 2); ++n) {
        iread(n, &ino);
        linkmap[n] = -1;
        if (ino.i_mode == 0)
            continue;
        ...
        if (mode == F_REG || mode == F_DIR) {
            ...
            for (b = 18; b < 20; ++b) {
                ...
                if (ino.i_addr[b] != 0 && swizzle32(ino.i_size) < 18*400) {
            ...
            /* Check the rest */
            for (bno = 0; bno <= swizzle32(ino.i_size)/400; ++bno) {
                b = getblkno(&ino, bno);
    ...
    if (swizzle16(superblock.s_tinode) != 6 * (swizzle16(superblock.s_isize) - 2) - ROOTINODE - icount) {
        printf("Free inode count in superblock block is %u, should be %u. Fix? ",
                swizzle16(superblock.s_tinode), 6 * (swizzle16(superblock.s_isize) - 2) - ROOTINODE - icount);

        if (yes()) {
            superblock.s_tinode = swizzle16(6 * (swizzle16(superblock.s_isize) - 2) - ROOTINODE - icount);
            dwrite((blkno_t) 1, (uint8_t *) &superblock);
        }
    }
}
...
static void ckdir(uint16_t inum, uint16_t pnum, char *name)
{
    ...
    if (swizzle32(ino.i_size) % 40 != 0) {
        printf("Directory inode %u has improper length %d. Fix? ", inum, swizzle32(ino.i_size));
        if (yes()) {
            ino.i_size = swizzle32(40 * (swizzle32(ino.i_size) / 40));
            iwrite(inum, &ino);
        }
    }
    nentries = swizzle32(ino.i_size)/40;

    for (j = 0; j < nentries; ++j) {
        dirread(&ino, j, &dentry);

        /* FIXME: rewrite if changed only */
        ...
        if (swizzle16(dentry.d_ino) < ROOTINODE ||
                swizzle16(dentry.d_ino) >= 6 * swizzle16(superblock.s_isize) - 2) {
...
static void mkentry(uint16_t inum)
{
    ...
    for (d = 0; d < swizzle32(rootino.i_size)/40; ++d) {
...
static blkno_t getblkno(struct dinode *ino, blkno_t num)
{
    ...
    dindb = swizzle16(buf[(num - (18 + 200)) / 200]);
    if (dindb == 0)
        return 0;

    buf = (blkno_t *) daread(dindb);

    return swizzle16(buf[(num - (18 + 200)) % 200]);
}
...
static void setblkno(struct dinode *ino, blkno_t num, blkno_t dnum)
{
    ...
    if (num < 18) {			/* Direct block */
    ...
    } else if (num < 200 + 18) {	/* Single indirect */
        indb = swizzle16(ino->i_addr[18]);
        if (indb == 0)
        ...
        buf = (blkno_t *) daread(dindb);
        buf[(num - (18 + 200)) % 200] = swizzle16(num);
        dwrite(indb, (uint8_t *) buf);
    }
}
...
/* The 400 byte fs uses 384 bytes of each inode block for 6 inodes then
   wastes a few bytes */
static void iread(uint16_t ino, struct dinode *buf)
{
    struct dinode *addr;

    addr = (struct dinode *) daread((ino / 6) + 2);
    bcopy((char *) &addr[ino % 6], (char *) buf, sizeof(struct dinode));
}
...
static void iwrite(uint16_t ino, struct dinode *buf)
{
    ...
    addr = (struct dinode *) daread((ino / 6) + 2);
    bcopy((char *) buf, (char *) &addr[ino % 6], sizeof(struct dinode));
    dwrite((ino / 6) + 2, (uint8_t *) addr);
}

/* Directory entries are 40 bytes padded for alignment reasons. We don't make
   the name length longer to keep compatibility */
static void dirread(struct dinode *ino, uint16_t j, struct direct *dentry)
{
    blkno_t blkno;
    uint8_t *buf;

    blkno = getblkno(ino, (blkno_t) j / 10);
    if (blkno == 0)
        panic("Missing block in directory");
    buf = daread(blkno);
    bcopy(buf + 40 * (j % 10), (char *) dentry, 40);
}
...
static void dirwrite(struct dinode *ino, uint16_t j, struct direct *dentry)
{
    ...
    if (blkno == 0)
        panic("Missing block in directory");
    buf = daread(blkno);
    bcopy((char *) dentry, buf + 40 * (j % 10), 40);
    dwrite(blkno, buf);
}
