...
#define NUM_ALLOCATION_BLOCKS (USERMEM / 4096)
static uint8_t allocation_map[NUM_ALLOCATION_BLOCKS] = {0};

static int get_slot(ptptr p)
{
	return p - ptab;
}
...
int pagemap_alloc(ptptr p)
{
	...
	kprintf("alloc %d, %d block\n", get_slot(p), (p->p_top - PROGLOAD) / 4096);
	if (udata.u_ptab && udata.u_ptab->p_page)
	{
		kprintf("evicting %d\n", get_slot(udata.u_ptab));
		swapout(udata.u_ptab);
		udata.u_ptab->p_page = 0;
	}
	p->p_page = 1;
	kprintf("done alloc\n");
	return 0;
...
{
    kprintf("realloc %d to %p bytes\n", get_slot(udata.u_ptab), size);
