...
/*                                                                       */
/* Updated for SC111 to use bit 0 of port A on the PIO as the CS         */
/* Updated for SC126 to use GPIO interface at port 0x0C			 */
/* TODO: more debugging and reduce the M1 pokes to the minimum needed    */
/*-----------------------------------------------------------------------*/
...
__sfr __at 0x68 pio0_portad;
__sfr __at 0x69 pio0_portbd;
__sfr __at 0x6A pio0_portac;
__sfr __at 0x6B pio0_portbc;
...
/* Keep all the unused pins as inputs */
void pio_init(void)
{
    if (is_sc126)
      return;
    pio0_portac = 0xCF;		/* mode 3 */
    Z180_OMCR &= ~0x40;
    pio0_portac = 0xFE;		/* for now just using A0 as an output */
    Z180_OMCR &= ~0x40;
    pio0_portac = 0x07;		/* interrupt off */
    Z180_OMCR &= ~0x40;
    pio0_portbc = 0xCF;		/* park port B as inputs */
    Z180_OMCR &= ~0x40;
    pio0_portbc = 0xFF;
    Z180_OMCR &= ~0x40;
    pio0_portbc = 0x07;
    Z180_OMCR &= ~0x40;
    ASCI_STAT1 &= ~0x04;	/* CTS1E mux off, pin is RXS */
    pio0_portad = 1;
    Z180_OMCR &= ~0x40;
//    kprintf("%x ", pio0_portad);
    pio0_portad = 0;
    Z180_OMCR &= ~0x40;
//    kprintf("%x ", pio0_portad);
}
