...
static int devfd_transfer(uint_fast8_t minor, bool is_read, uint_fast8_t rawflag)
{
    int ct = 0;
    int tries;
    int blocks;
    ...
    /* kprintf("[%s %d @ %x : %d:%x]\n", is_read ? "read" : "write",
          blocks, lba, rawflag, udata.u_dptr);
       if (!is_read)
...
{
    flag;minor;
    return devfd_transfer(minor, true, rawflag);
}

int devfd_write(uint_fast8_t minor, uint_fast8_t rawflag, uint_fast8_t flag)
{
    flag;minor;
    return devfd_transfer(minor, false, rawflag);
}
