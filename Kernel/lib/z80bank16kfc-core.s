...
skip_copyback:
	...
	; Unlike fixedbank we use 3 bytes so this is the conditional bit
	; when we unify them in 0.4 FIXME
	ld l, P_TAB__P_PAGE_OFFSET(ix)
	ld h, P_TAB__P_PAGE_OFFSET+1(ix)
	ld (_udata + U_DATA__U_PAGE), hl
	ld a, P_TAB__P_PAGE_OFFSET+2(ix)
